...
def bounding_box(mp: MapPoint, length=20):
    """
    calculates the bounding coordinates provided a MapPoint and distance in meters from the point.
    :param mp: MapPoint object around which to create a bounding box
    :param length: distance from center point to a single side of square. half the length of a side.
    :return: dict of n,s,e,w geo coordinates
    """
    # S-, N+
    lats = []
    # W-, E+
    ...
    for x in range(0, 360, 360//4):
        new_mp = calcCoordinate(mp, length, x)
